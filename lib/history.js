...
function toSnapshots(rows, cols, opts = {}) {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      // Seul `navigation.headingTrue` est demandé au magasin (voir
      // CANDIDATES) : la provenance ne fait aucun doute, et l'écrire évite que
      // lib/leeway.js ait à deviner ce qu'un champ absent voulait dire.
      hdgSrc: cols.hdg !== undefined ? 'true' : null,
