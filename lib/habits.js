...
function summarise(runs) {
  ...
  const twsKn = usable.map((r) => r.tws * KN);
  ...
  const sog = usable.map((r) => (r.sog || 0) * KN).sort((a, b) => a - b);
  const tws = twsKn.slice().sort((a, b) => a - b);
  const heel = usable
...
module.exports = { summarise, tally, bandOf, isNight, POINTS_OF_SAIL, WIND_BANDS, KN };
