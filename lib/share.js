...
// Trois états, pas deux. « Coupé » et « jamais renseigné » se ressemblent de
// loin — rien ne part, dans les deux cas — mais ils appellent l'inverse l'un
// de l'autre : le premier est une décision prise, qu'on respecte en se
// taisant ; le second est un formulaire que personne n'a rempli, et c'est la
// seule population à qui il reste quelque chose à dire. Les confondre dans le
// ping revenait à compter comme des refus des gens qui n'avaient jamais été
// mis devant la question.
function stateOf(opts) {
  const o = opts || {};
  if (!o.sharePolar) return 'off';
  if (!o.boatModel || !o.shareName) return 'unconfigured';
  return 'on';
}
...
function createShare(file, log) {
  ...
  function maybeSend(opts, count, build) {
    if (stateOf(opts) !== 'on' || !opts.shareEndpoint) return false;
...
module.exports = { createShare, isDue, stateOf, RETRY_MS };
