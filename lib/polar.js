...
const leewayLib = require('./leeway');
...
const DEFAULTS = {
  speed: 'sog',
  wind: 'true',
  // Le repère dans lequel on lit l'angle du vent.
  //   'water'  — l'angle mesuré, entre l'axe du bateau et le vent vrai.
  //              C'est l'angle qu'on barre, et celui de toutes les polaires.
  //   'ground' — l'angle entre la ROUTE SUR LE FOND et le vent, dérive
  //              déduite. C'est celui qui dit où le bateau arrive vraiment :
  //              pincer à 35° pour faire 43° sur le fond ne vaut pas mieux que
  //              tenir 40° sans déraper, et seule cette lecture-là le montre.
  // Le vent apparent n'a pas de version « fond » : il est mesuré par rapport
  // à l'axe du bateau, un mât ne sait rien de la route fond.
  angle: 'water',
  // Le modèle de dérive de lib/leeway.js (analyse()). Sans lui, la lecture
  // 'ground' n'est pas disponible : on ne devine pas une dérive.
  leeway: null,
  tack: 'merged',
  // La médiane, pas la moyenne. La dispersion dans une case vient de la mer
  // et des rafales, pas d'une variation de performance : ce qu'on veut, c'est
  // la valeur qui ne bouge pas quand un point est mauvais. Un surf sur une
  // vague déplace une moyenne ; il ne déplace pas une médiane. Et pour une
  // ETA, « ce que le bateau tient » vaut mieux que « ce qu'il a fait de
  // mieux » — p90 et max restent disponibles, comme lecture, pas par défaut.
  stat: 'median',
  ...
  // Écarte les points navigués avec une voile explicitement absente — sans
  // grand-voile, sans voile d'avant. Voir isTestRig() : ces points décrivent
  // un bateau qu'on essaie, pas celui qu'on route.
  excludeTestRig: false,
  smooth: true,
...
function project(run, o) {
  ...
  let wa = o.wind === 'apparent' ? run.awa : run.twa;
  // Lecture « sur le fond » : l'angle est repris depuis la route fond du point
  // lui-même, pas corrigé par une moyenne. Un point sans cap exploitable n'est
  // pas rattrapé par le modèle — il sort de la polaire, et le compteur le dit.
  if (groundMode(o)) wa = leewayLib.groundTwa(run, o.leeway);
  if (typeof speed !== 'number' || typeof ws !== 'number' || typeof wa !== 'number') return null;
  return { speed, ws, wa: wrap180(wa) };
}

// La lecture fond n'a de sens que sur le vent vrai, et seulement si la dérive
// a vraiment été mesurée. Partout ailleurs on retombe silencieusement sur
// l'angle mesuré : mieux vaut la polaire habituelle qu'une polaire corrigée
// par une dérive devinée.
function groundMode(o) {
  return o.angle === 'ground' && o.wind !== 'apparent' && !!(o.leeway && o.leeway.usable);
}
...
// Une voilure d'essai : une voile explicitement ABSENTE. À ne pas confondre
// avec « non renseigné » (chaîne vide), qui veut seulement dire que personne
// n'a posé l'étiquette — c'est le cas de l'immense majorité des points, et ce
// n'est certainement pas une raison de les écarter.
//
// La distinction existe parce qu'on ne navigue pas sans grand-voile par
// hasard : on le fait pour essayer quelque chose. Ces points sont de vraies
// mesures, ils restent collectés et consultables, mais un week-end d'essais
// au seul génois ne doit pas creuser durablement la table qui sert à router.
function isTestRig(sail) {
  const s = normalizeSail(sail);
  return s.main === 'none' || String(s.head).split('-')[0] === 'none';
}
...
function bucket(runs, opts) {
  ...
  // Sur quoi reposait « pas au moteur », pour les points RETENUS et eux seuls.
  // C'est compté ici et pas sur `runs` parce que c'est cette polaire-là qu'on
  // partage : le décompte fait sur tous les runs annonçait au fonds commun des
  // points `declared` dans une polaire qui, par construction, les exclut.
  const engineSources = {};
  let used = 0;
  let rejected = 0;
  // En lecture fond, un point dont le cap manque (ou dont l'écart route/cap
  // est aberrant) n'a pas d'angle : il disparaît de la polaire. Ça doit se
  // voir, sinon on compare deux courbes qui ne reposent pas sur les mêmes
  // points sans le savoir.
  let noAngle = 0;
  ...
  for (const run of runs) {
    ...
    if (o.excludeTestRig && isTestRig(effectiveSail(run, o.sailRanges))) continue;
    if (!matchSail(run, o.sail, o.sailRanges)) continue;
    const p = project(run, o);
    if (!p) {
      if (groundMode(o) && leewayLib.groundTwa(run, o.leeway) == null && typeof run.twa === 'number') noAngle++;
      continue;
    }
    ...
    const src = run.engineSource || 'unknown';
    engineSources[src] = (engineSources[src] || 0) + 1;
    used++;
  }
  return { o, edges, centers, cells, used, engineSources, rejected, noAngle, total: runs.length };
}

function buildPolar(runs, opts) {
  const { o, edges, centers, cells, used, engineSources, rejected, noAngle, total } = bucket(runs, opts);
  ...
  const bins = edges.map((e) => {
    ...
    const targets = targetsFor(cellList, o.vmgOffsets);
    annotateGround(targets, o);
    return { ws: e.ws, lo: e.lo, hi: e.hi, cells: cellList, targets };
  });

  return {
    speed: o.speed,
    wind: o.wind,
    angle: groundMode(o) ? 'ground' : 'water',
    // Demandé mais pas rendu : la webapp doit pouvoir dire POURQUOI elle
    // affiche encore l'angle mesuré alors que le bouton « over ground » est
    // enfoncé, au lieu de laisser croire que la correction est appliquée.
    angleAsked: o.angle === 'ground' ? 'ground' : 'water',
    tack: o.tack,
    stat: o.stat,
    twaStep: o.twaStep,
    twsBins: o.twsBins,
...
// Ce que la cible devient une fois la dérive prise en compte : le même point,
// lu dans le repère où l'on arrive quelque part. On n'y touche pas aux
// valeurs — c'est une annotation, pas une correction — mais c'est elle qui
// répond à « 35° du vent, oui, mais 43° sur le fond ».
//
// En lecture fond, l'inverse : les angles SONT ceux du fond, et l'annotation
// rend l'angle à barrer.
function annotateGround(targets, o) {
  const model = o.leeway;
  if (!model || !model.usable) return;
  const ground = groundMode(o);
  for (const key of ['upwind', 'downwind']) {
    const t = targets[key];
    if (!t || t.speed == null) continue;
    // Jamais de dérive négative : au portant la mesure oscille autour de zéro,
    // et un zéro bruité négatif « refermerait » l'angle, ce qui n'existe pas.
    const lee = Math.max(0, leewayLib.leewayAt(model.curve, t.twa));
    if (ground) {
      t.leeway = lee;
      t.twaWater = Math.max(0, t.twa - lee);
    } else {
      t.leeway = lee;
      t.twaGround = Math.min(180, t.twa + lee);
      t.vmgGround = Math.abs(t.speed * Math.cos((t.twaGround * Math.PI) / 180));
    }
  }
}
...
function cellPoints(runs, opts, ws, twa) {
  ...
  return pts.map((p) => ({
    ...
    aws: p.run.aws,
    awa: p.run.awa,
    // L'écart route fond / cap du point, et l'angle qui en découle. Une case
    // qui sort du lot s'explique souvent là : un point relevé en plein
    // courant, ou juste après un virement, dérive autrement que les autres.
    offset: leewayLib.offsetOf(p.run),
    twaGround: opts && opts.leeway ? leewayLib.groundTwa(p.run, opts.leeway) : null,
    roll: p.run.roll,
    pitch: p.run.pitch,
...
// ── Exports ────────────────────────────────────────────────────────────────
// Un fichier de polaire se retrouve six mois plus tard sans le souvenir des
// boutons enfoncés le jour du téléchargement. Les angles « sur le fond » ne se
// barrent pas : un routeur qui les prendrait pour des angles de barre serait
// faussé de la valeur de la dérive. Ça s'écrit donc DANS le fichier.
const frameNote = (polar) =>
  polar.angle === 'ground' ? ' / wind angle measured from the track over ground (leeway removed)' : '';
...
function toJieter(polar) {
  ...
  const lines = [
    `# signalk-autopolar — ${polar.speed.toUpperCase()} / ${polar.wind} wind / ${polar.stat} per cell${frameNote(polar)}`,
...
function toCsv(polar) {
  ...
  // Comme le Jieter : la projection est écrite dans le fichier. Le CSV est
  // l'export qu'on ouvre dans un tableur six mois plus tard, quand on ne sait
  // plus quels boutons étaient enfoncés le jour du téléchargement. Les
  // colonnes `_n` disent le nombre de mesures derrière chaque valeur : une
  // case à 1 mesure n'est pas une mesure de la même nature qu'une case à 80.
  const lines = [
    `# signalk-autopolar — ${polar.speed.toUpperCase()} / ${polar.wind} wind / ${polar.stat} per cell${frameNote(polar)}`,
    head,
  ];
