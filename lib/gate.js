...
function classify(snap, opts) {
  ...
  // tourne. C'est une nuance qui compte : `propulsion.*.revolutions` est en
  // hertz selon la spec, mais rien n'oblige une passerelle à la respecter, et
  // une valeur mal mise à l'échelle peut faire passer un moteur à l'arrêt pour
  // un moteur au ralenti. `propulsion.*.state` répond directement à la
  // question et ne dépend d'aucune unité : c'est lui qu'on croit en premier.
  //
  ...
  if (hasState || hasRpm) {
    ...
    const runningByRpm = hasRpm && snap.rpm > opts.engineOffRpm;
